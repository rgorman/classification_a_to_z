---
title: "XML pre-processing, Variable extraction and Combination"
...
This section describes how XML treebank files are prepared for analysis.
...
* **For group 1,** all possible combinations are generated. With 18 variables to be combined, the number of resultant combination is   $\sum_{i=1}^{18} \binom{18}{i}$ = 262,143  
...
* **After removing impossible attribute combintions,** a selection including all of Group 1, and $\binom{n}{1}$ through $\binom{n}{5}$ for each of the other five groups yields a total of 572,571 combinations of attributes to serve as new variables.
...
```{r}
a <- unlist(lapply(culled.output.list[1:3], ncol))
v.list <- vector("list", sum(a))
counter <- 1
var.names <- NULL

t1 <- Sys.time()
for (i in 1:3) { # Note well: This loop is truncated for testing.  The full loop will take a long time.
